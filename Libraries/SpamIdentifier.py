...
class SpamIdentifier:
    def __init__(self) -> None:
        ...
        self.__generated_file = "../data/processed/spam_classification.csv"
        ...
        if os.path.exists(self.__generated_file):
            ...
            self.__orig_df = pd.read_csv("../data/raw/mail_data.csv")

            # Data Preparation
            self.__orig_df["IsSpam"] = self.__orig_df["Category"].map(
                {"spam": 1, "ham": 0}
            )
    ...
    def Algorithm_Naive_Bayes_MultinomialNB(self):
        ...
        for i in range(1, repeatCount, 1):
            x_train, x_test, y_train, y_test = train_test_split(
                self.__df["Message"], self.__df["IsSpam"], test_size=0.20
            )
